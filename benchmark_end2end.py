...
parser.add_argument('--attention_type', type=str, default='sdpa', choices=['sdpa', 'sage', 'fa3', 'fa3_fp8', 'sparge', 'flashinfer', 'sta'], help='Attention type')
...
if args.attention_type == 'sage':
    ...
    from flash_attn.flash_attn_interface import flash_attn_func as fa3
    F.scaled_dot_product_attention = fa3
elif args.attention_type == 'fa3_fp8':
    from flash_attn.flash_attn_interface import flash_attn_func_fp8 as fa3_fp8
    ...
    from flashinfer import flashinfer_attn
    F.scaled_dot_product_attention = flashinfer_attn
elif args.attention_type == 'sta':
    from sta import sta_attn
...
with torch.no_grad():
    for _ in range(args.warmup):
        _ = pipe(
            ...
            num_inference_steps=5,   # Use shorter steps/frames for warmup to save time
            num_frames=8,
            guidance_scale=6,
